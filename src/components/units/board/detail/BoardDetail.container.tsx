...
export default function BoardDetail() {
  ...
  useEffect(() => {
    ...
    if (typeof boardId === "string") {
      ...
      axios
        .get(`http://localhost:8080/api/posts/view/${boardId}`)
        .then((response) => {
  ...
  const onClickDelete = async () => {
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      // 삭제 확인 메시지를 표시합니다.
      try {
        await axios.delete(`http://localhost:8080/api/posts/delete/${boardId}`); // 게시글 삭제 API 호출
        router.push("/boards"); // 게시글 목록 페이지로 이동
      } catch (error) {
        console.error("게시물을 삭제하는 중 에러 발생:", error); // API 호출 중 오류가 발생하면 콘솔에 오류를 출력합니다.
      }
    }
  };
